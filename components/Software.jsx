...
function Software() {
  return (
    <div className="flex flex-col gap-6 p-8 bg-[#051139] w-full h-full items-center justify-center relative overflow-hidden">
      <div className="absolute z-0 left-2 top-0 rounded-[100%] w-[400px] h-[340px] bg-[#1E50FF] blur-[100px] -translate-y-8"></div>
      <div className="absolute z-0 right-12 -bottom-0 rounded-[100%] w-[400px] h-[350px] bg-[#9C08FF] blur-[120px] translate-y-8"></div>
      ...
      <div className="flex flex-wrap w-full justify-center gap-6 md:gap-8 mt-20 relative z-10">
        {[<FaPython />, <FaSquare />, <TbBrandJavascript />, <IoLogoFigma />, <BsFillPentagonFill />, <TbOvalVerticalFilled />, <VscVscode />].map((icon, index) => (
          <div key={index} className="text-xl md:text-3xl text-gray-300">
            {icon}
          </div>
        ))}
      </div>
      <div className="flex flex-wrap w-full justify-center gap-6 md:gap-8 mb-24 mt-4 relative z-10">
        {[<TbBrandGithubFilled />, <PiTriangleDashedFill />, <VscVscode />, <PiStarFourFill />, <FaCubes />, <BsFillHexagonFill />, <SiFlutter />].map((icon, index) => (
          <div key={index} className="text-xl md:text-3xl text-gray-300">
            {icon}
          </div>
        ))}
